<template>
    <div class="animate-fade-in relative">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            <div v-for="item in paginatedStock" :key="item.id" class="bg-neutral-800/80 rounded-xl border border-neutral-700/50 overflow-hidden hover:border-amber-500/50 transition-all group shadow-lg flex flex-col relative">
                ...
                <div class="p-4 flex-1 flex flex-col">
                    ...
                    <div v-if="item.unit === 'case'" class="text-[11px] text-neutral-500 mb-3 flex justify-between font-bold">
                        <span>១ កេះស្មើនឹង</span>
                        <span class="text-amber-400">{{ item.itemsPerCase }} ដប/កញ្ចប់</span>
                    </div>
        ...
        <div v-if="totalPages > 1" class="flex justify-center items-center gap-3 mt-8">
            ...
            <span class="px-4 py-2 bg-neutral-900 rounded-lg border border-neutral-700 text-amber-500 font-black">{{ currentPage }} / {{ totalPages }}</span>
            <button @click="$emit('page-change', currentPage + 1)" :disabled="currentPage === totalPages" class="px-4 py-2 bg-neutral-800 rounded-lg hover:bg-neutral-700 disabled:opacity-50 text-sm font-bold transition-colors">បន្ទាប់</button>
        </div>
            ...
            <div v-if="isEditModalOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" @click.self="closeEditModal">
                <div class="bg-neutral-900 border border-neutral-700 rounded-2xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden animate-slide-up">
                    ...
                    <div class="p-6 overflow-y-auto custom-scrollbar max-h-[70vh]">
                        <div class="mb-4">
                            <p class="text-xs text-neutral-400 mb-1">ឈ្មោះទំនិញ:</p>
        ...
        items.forEach((item, index) => {
            ...
            tableRows += `
                <tr style="break-inside: avoid; page-break-inside: avoid;">
                    <td style="padding: 10px 8px; text-align: center; color: #64748b; font-weight: bold; width: 8%; border-bottom: 1px solid #e2e8f0; border-right: 1px solid #e2e8f0;">${index + 1}</td>
                    <td style="padding: 10px 8px; text-align: center; width: 12%; border-bottom: 1px solid #e2e8f0; border-right: 1px solid #e2e8f0;">${imgSrc}</td>
                    <td style="padding: 10px 12px; color: #0f172a; font-weight: bold; width: 45%; border-bottom: 1px solid #e2e8f0; border-right: 1px solid #e2e8f0; font-size: 13px; line-height: 1.4;">${item.name || 'មិនមានឈ្មោះ'}</td>
                    <td style="padding: 10px 12px; text-align: center; font-family: monospace; font-weight: 900; color: #4f46e5; letter-spacing: 1px; font-size: 14px; width: 35%; border-bottom: 1px solid #e2e8f0;">${item.barcode || '-'}</td>
                </tr>
            `;
        });
                ...
                <table style="width: 100%; border-collapse: collapse; border: 1px solid #e2e8f0;">
                    <thead style="background-color: #f8fafc;">
                        <tr>
                            ...
                            <th style="padding: 12px 12px; text-align: left; color: #334155; font-size: 12px; font-weight: 900; border-bottom: 2px solid #cbd5e1; border-right: 1px solid #e2e8f0;">ឈ្មោះទំនិញ</th>
                            <th style="padding: 12px 12px; text-align: center; color: #334155; font-size: 12px; font-weight: 900; border-bottom: 2px solid #cbd5e1;">បាកូដ (Barcode)</th>
                        </tr>
                    </thead>
...
// --- QUICK EDIT MODAL LOGIC WITH SMART LOCKDOWN ---
const isEditModalOpen = ref(false);
const selectedItem = ref(null);
const isSaving = ref(false);
const fileInputModal = ref(null);

// 🌟 States ថ្មីសម្រាប់ការពារការកែប្រែ 🌟
...
const openEditModal = (item) => {
    ...
    // 🌟 ត្រួតពិនិត្យប្រវត្តិ IN ជាបន្ទាន់ (Realtime) 🌟
    ...
    historyCheckUnsubscribe = onSnapshot(q, (snapshot) => {
        // បើមាន Record យ៉ាងហោច ១ គឺចាក់សោរ
...
const saveQuickEdit = async () => {
    ...
    try {
        ...
        // 🌟 ១. Update រូបភាពជានិច្ចបើមានការដូរ
        if (editForm.imagePreview) {
            updateData.image = editForm.imagePreview;
        }

        // 🌟 ២. បើគ្មានប្រវត្តិ IN សោះ ទើបអនុញ្ញាតឱ្យ Update ចំនួន ទុន និងខ្នាត
        ...
        // 🌟 ៣. អនុញ្ញាតឱ្យ Update ថ្ងៃខែ តែបើវានៅទទេពីមុន (ឬបើមិនទាន់មានប្រវត្តិ)
