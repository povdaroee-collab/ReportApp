                        ...
                        >
                            ...
                            <template v-else>
                                <td class="px-6 py-4 align-top pt-6">
                                    <span v-if="seller.hasSales" class="font-bold text-slate-800 text-sm">
                                        {{ seller.productName }}
                                    </span>
                                ...
                                <td class="px-6 py-4 text-center align-top pt-6">
                                    <span v-if="seller.hasSales" :class="seller.saleType === 'បោះដុំ' ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-slate-50 text-slate-700 border-slate-200'" class="px-2.5 py-1 rounded-md text-[10px] font-bold border tracking-wide">
                                        {{ seller.saleType }}
                                    </span>
                                    <span v-else class="text-slate-300 font-bold">-</span>
                                </td>
...
const calculatedSellers = computed(() => {
    ...
    allSellers.value.forEach(seller => {
        ...
        // Match sales for this admin AND this seller
        const relevantSales = allSales.value.filter(sale => {
            const isAdminMatch = sale.createdBy === admin.value.id; 
            const saleSellerId = sale.uid || sale.sellerId;
            const isSellerMatch = saleSellerId === seller.id || sale.sellerName === seller.fullName; 
            const dateStr = sale.createdAt || sale.date;
            
            return isAdminMatch && isSellerMatch && isDateInScope(dateStr);
        });

        // 🔴 Logic សម្រាប់ Tab "បញ្ជីតំណាងលក់" (Sellers List)
        ...
        // 🟢 Logic សម្រាប់ Tab "របាយការណ៍លក់" (Sales Report)
        // Group by Item Name + Sale Type
        let hasValidSales = false;
        let itemGroups = {}; // key: "ProductName_SaleType"

        relevantSales.forEach(sale => {
            (sale.items || []).forEach(item => {
                let itemCategory = item.type === 'wholesale' ? 'បោះដុំ' : 'លក់រាយ';

                // Check active filter (All, Retail, Wholesale)
                if (activeCategory.value === 'all' || activeCategory.value === itemCategory) {
                    hasValidSales = true;
                    
                    let pName = item.name || 'មិនស្គាល់ឈ្មោះ';
                    let key = `${pName}_${itemCategory}`;

                    // Initialize group if not exists
                    if (!itemGroups[key]) {
                        itemGroups[key] = {
                            productName: pName,
                            saleType: itemCategory,
                            unitCounts: {},
                            revenueUSD: 0,
                            revenueKHR: 0,
                            clients: 0
                    ...
                    if (!itemGroups[key].unitCounts[u]) itemGroups[key].unitCounts[u] = 0;
                    itemGroups[key].unitCounts[u] += Number(item.qty || 0);

                    // Sum revenue
                    const itemRevenue = Number(item.qty || 0) * Number(item.price || 0);
                    const currency = sale.currency || 'USD';
                    if (currency === 'USD' || currency === '$') {
                        itemGroups[key].revenueUSD += itemRevenue;
                    } else {
                        itemGroups[key].revenueKHR += itemRevenue;
                    }

                    // Count Clients (Assume 1 item line from 1 receipt = 1 client for this group)
                    itemGroups[key].clients += 1;
                }
            });
        });

        // Push grouped data as separate rows
        if (hasValidSales) {
            Object.values(itemGroups).forEach((group, index) => {
                rows.push({ 
                    ...seller, 
                    uniqueId: `${seller.id}_report_${index}`, 
                    productName: group.productName,
                    saleType: group.saleType,
                    totalClients: group.clients, 
                    revenueUSD: group.revenueUSD, 
                    revenueKHR: group.revenueKHR, 
                    unitCounts: group.unitCounts, 
                    hasSales: true 
                });
            });
        } else if (activeCategory.value === 'all') {
            // Push empty seller row if no sales at all
            rows.push({ 
                ...seller, 
                uniqueId: `${seller.id}_empty`, 
                hasSales: false 
            });
        }
    });
...
const filteredSellers = computed(() => {
    ...
    return calculatedSellers.value.filter(s => 
        ...
        (s.productName && s.productName.toLowerCase().includes(q)) // Can search by product name in report view
    );
});
...
const grandTotals = computed(() => {
    ...
    if (activeTab.value === 'sellers') return stats; // Don't calculate for seller list
...
// --- PRINT & PDF (100% UNCHANGED AS REQUESTED, ONLY UPDATED HEADERS TO MATCH NEW COLUMNS) ---
const executePrint = () => {
...
const generatePrintHTML = (rows, pageNum, totalPages, isNativePrint = false) => {
    ...
    const renderRow = (item) => {
        if (isReport) {
            ...
            const catBadge = item.saleType === 'បោះដុំ' 
