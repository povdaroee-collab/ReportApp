        ...
        <div class="flex flex-col md:flex-row gap-6 relative z-10">
            ...
            <div class="flex-1">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-bold text-white">តំណាងលក់សរុប: {{ totalSellersCount }} នាក់</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="flex flex-col lg:flex-row justify-between items-center gap-4 mb-6 print:hidden">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto bg-white p-2 rounded-2xl shadow-sm border border-slate-200 shrink-0 z-20">
            <div class="relative w-full sm:w-64">
                ...
                <input :value="searchQuery" @input="$emit('update:searchQuery', $event.target.value)" type="text" placeholder="ស្វែងរកឈ្មោះ ឬ អត្តលេខ..." class="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:bg-white focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all placeholder:font-normal">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto justify-end">
                ...
                <div v-if="activeTab === 'report'" class="bg-slate-100 p-1.5 rounded-xl flex border border-slate-200 animate-fade-in shrink-0">
                    <button @click="$emit('update:activeCategory', 'all')" :class="activeCategory === 'all' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'" class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all">ទាំងអស់</button>
                    <button @click="$emit('update:activeCategory', 'បោះដុំ')" :class="activeCategory === 'បោះដុំ' ? 'bg-white shadow-sm text-purple-700' : 'text-slate-500 hover:text-purple-700'" class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all">បោះដុំ</button>
                    <button @click="$emit('update:activeCategory', 'លក់រាយ')" :class="activeCategory === 'លក់រាយ' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-indigo-600'" class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all">លក់រាយ</button>
                </div>

                <button @click="$emit('print')" :disabled="filteredSellersLength === 0" class="bg-slate-800 hover:bg-slate-900 text-white px-3 py-2 rounded-xl text-xs font-bold shadow-md transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-1.5 shrink-0">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2-2v4h10z"></path></svg>
                    Print
                </button>
                <button @click="$emit('pdf')" :disabled="filteredSellersLength === 0 || processing.active" class="bg-rose-500 hover:bg-rose-600 text-white px-3 py-2 rounded-xl text-xs font-bold shadow-md shadow-rose-500/30 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-1.5 shrink-0">
