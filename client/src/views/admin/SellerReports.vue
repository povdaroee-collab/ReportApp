            ...
            <div v-if="isEditing" class="mb-8 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/60 rounded-2xl p-5 flex items-start gap-4 animate-fade-in shadow-sm">
                ...
                <div>
                   ...
                   <p class="text-[11px] font-bold text-amber-600 mt-1 leading-relaxed">
                      អ្នកកំពុងកែប្រែទិន្នន័យលក់ <strong class="text-rose-500">"{{ translateUnit(form.unit) }}"</strong> របស់តំណាងលក់នេះ។ ការរក្សាទុកនឹងធ្វើការ <strong class="text-amber-800">កែប្រែទិន្នន័យចាស់ជាន់ពីលើ</strong>។
                   </p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-8">
              ...
              <div class="lg:col-span-2 pt-6 mt-2 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-center justify-end gap-3 sm:gap-4">
                ...
                >
                  ...
                  <svg v-else-if="isEditing" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
...
// 🚀 WATCHER: CHECKS SELLER + DATE + UNIT
  ...
  if (newId && newDate && newUnit) {
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        triggerAlert('info', 'របៀបកែប្រែ', `រកឃើញទិន្នន័យចាស់សម្រាប់ ${translateUnit(newUnit)}។`);
      } else {
