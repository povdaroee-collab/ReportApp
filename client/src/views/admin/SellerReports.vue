         ...
         <div v-else>
            ...
            <div class="md:hidden flex flex-col gap-4">
               <div v-for="(item, index) in displayedData" :key="item.id" class="bg-white p-5 rounded-[20px] shadow-[0_4px_15px_rgb(0,0,0,0.03)] border relative overflow-hidden group transition-all" :class="item.hasSales ? 'border-slate-100' : 'border-rose-100/50 bg-slate-50/50'">
                      ...
                      <div v-if="item.hasSales" class="bg-slate-50/80 rounded-xl p-4 grid grid-cols-2 gap-4 border border-slate-100/80">
                         <div>
                            ...
                            <div class="flex flex-col gap-1.5">
                               <span v-for="(count, unit) in item.unitCounts" :key="unit" class="text-[11px] font-bold px-2.5 py-1 rounded-md border bg-white shadow-sm w-fit" :class="getUnitBadgeColor(unit)">
                                  {{ count.toLocaleString() }} {{ translateUnit(unit) }}
                               </span>
                            </div>
                         </div>
                  ...
                  <div v-else class="bg-indigo-50/50 rounded-xl p-4 border border-indigo-100 animate-fade-in space-y-4">
                      <div>
                      ...
                      <div class="flex gap-2 pt-2 mt-2 border-t border-indigo-100/60">
                         <button @click="cancelEdit" class="flex-1 py-2 rounded-lg bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 font-bold text-xs transition-colors">បោះបង់ (Cancel)</button>
                     ...
                     <tr v-for="(item, index) in displayedData" :key="item.id" class="transition-colors group" :class="[item.hasSales ? 'hover:bg-indigo-50/30' : 'bg-slate-50/30', editingSaleId === item.rawSale?.id ? 'bg-indigo-50/40 border-l-4 border-l-indigo-500' : '']">
                        ...
                        <td class="px-6 py-5 text-center">
                           <div class="flex items-center justify-center gap-2">
                              ...
                              <template v-else>
                                  <button @click="saveEdit" :disabled="isUpdatingSale" class="w-9 h-9 rounded-xl bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white flex items-center justify-center shadow-sm transition-all active:scale-95" title="រក្សាទុក (Save)">
                                  ...
                                  <button @click="cancelEdit" :disabled="isUpdatingSale" class="w-9 h-9 rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white flex items-center justify-center shadow-sm transition-all active:scale-95" title="បោះបង់ (Cancel)">
...
const sellers = ref([]);
const allSales = ref([]); 
const availableUnits = ref([]); // Dynamic units from DB
 ...
 onAuthStateChanged(auth, async (user) => {
  if (user) {
   try {
     ...
     // 4. Auto-select latest data if available
...
const baseCalculatedData = computed(() => {
 ...
 return sellers.value.map(seller => {
   ...
   // Store the FIRST raw sale object if it's daily view, so we can inline edit it
   // Note: Since this groups by seller, inline editing multiple units at once requires editing the specific document.
...
const saveEdit = async () => {
 ...
 try {
   ...
   editingSaleId.value = null; // Exit edit mode
