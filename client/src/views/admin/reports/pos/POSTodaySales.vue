<template>
  <div class="animate-fade-in pb-20 relative">
    ...
    <div class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-4 mb-4 bg-white p-3 sm:pl-5 sm:pr-3 rounded-[16px] shadow-sm border border-slate-200/80 w-full">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                <button @click="executeNativePrint" :disabled="processing.active || filteredSales.length === 0" class="flex-1 sm:flex-none bg-slate-800 hover:bg-slate-900 text-white border border-slate-800 px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-transform active:scale-95 font-bold text-xs shadow-sm disabled:opacity-50">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2-2v4h10z"></path></svg>
                    <span>Print</span>
                </button>
                <button @click="generatePDF" :disabled="processing.active || filteredSales.length === 0" class="flex-1 sm:flex-none bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-transform active:scale-95 font-bold text-xs disabled:opacity-50">
    ...
    <div v-if="isLoading" class="py-20 flex flex-col items-center justify-center opacity-60">
        <div class="w-10 h-10 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mb-3"></div>
        <p class="font-bold text-slate-500 text-sm">កំពុងទាញយកទិន្នន័យថ្ងៃនេះ...</p>
    </div>

    <div v-else-if="filteredSales.length === 0" class="py-24 bg-white/50 backdrop-blur-sm rounded-[24px] border-2 border-dashed border-slate-200 text-center animate-fade-in">
            ...
            <div class="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 cursor-pointer hover:bg-slate-50/50 transition-colors" @click="toggleExpand(sale.id)">
                ...
                <div class="flex items-center justify-between w-full sm:w-auto gap-4 border-t sm:border-none border-slate-100 pt-3 sm:pt-0">
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <button @click.stop="copyInvoiceText(sale)" class="p-2 bg-slate-50 hover:bg-emerald-50 text-slate-400 hover:text-emerald-600 border border-slate-100 hover:border-emerald-200 rounded-xl transition-all" title="ចម្លងអត្ថបទវិក្កយបត្រ">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                        </button>

                        <button @click.stop="saveInvoiceAsImage(sale)" class="p-2 bg-slate-50 hover:bg-blue-50 text-slate-400 hover:text-blue-600 border border-slate-100 hover:border-blue-200 rounded-xl transition-all" title="ទាញយករូបភាពវិក្កយបត្រ">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                        </button>

                        <button @click.stop="openEditModal(sale)" class="p-2 bg-slate-50 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 border border-slate-100 hover:border-indigo-200 rounded-xl transition-all" title="កែប្រែ">
    ...
    <div v-if="editModal.isOpen" class="fixed inset-0 z-[999] flex items-start justify-center p-4 pt-10 sm:pt-20 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
        <div class="bg-white rounded-[24px] w-full max-w-2xl shadow-2xl border border-white/20 overflow-hidden animate-slide-up flex flex-col max-h-[85vh]">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
                <h3 class="font-black text-slate-800 text-lg flex items-center gap-2">
                    ...
                    កែប្រែព័ត៌មានវិក្កយបត្រ <span class="text-indigo-500 text-sm ml-2">{{ editModal.receiptId }}</span>
                </h3>
                <button @click="editModal.isOpen = false" class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>
            ...
            <div class="p-6 flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input v-model="editForm.customerName" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5">លេខទូរស័ព្ទ</label>
                        <input v-model="editForm.customerPhone" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input v-model="editForm.province" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5">សាខា (ស្រុក/ខណ្ឌ/ឃុំ/សង្កាត់)</label>
                        <input v-model="editForm.district" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select v-model="editForm.paymentMethod" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all cursor-pointer">
                    ...
                    <div>
                        ...
                        <select v-model="editForm.paymentStatus" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all cursor-pointer">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input v-model="editForm.createdAt" type="datetime-local" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all cursor-pointer">
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5">ចំណាំបញ្ជាក់ការទូទាត់ (Payment Note)</label>
                        <input v-model="editForm.paymentNote" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all">
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50 shrink-0">
                ...
                <button @click="saveEdit" :disabled="isSaving" class="px-8 py-2.5 rounded-xl text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-500/30 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2">
    ...
    <div v-if="deleteModal.isOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
    ...
    <div v-if="cancelModal.isOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
    ...
    <div ref="printStaging" class="fixed top-0 left-[-9999px] pointer-events-none z-[-1] bg-white opacity-0 print:opacity-100"></div>
    <div ref="invoiceStaging" class="fixed top-0 left-[-9999px] pointer-events-none z-[-1] bg-white opacity-0 print:opacity-100"></div>

    <div v-if="processing.active" class="fixed inset-0 z-[999999] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-fade-in">
        <div class="w-full max-w-sm bg-white rounded-3xl p-8 shadow-2xl relative flex flex-col items-center text-center">
            <div class="w-16 h-16 border-4 border-indigo-100 border-t-indigo-600 rounded-full animate-spin mb-4"></div>
            <h3 class="text-lg font-black text-slate-800 mb-2">{{ processing.message }}</h3>
            <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden mt-2">
                <div class="h-full bg-indigo-500 transition-all duration-300" :style="{ width: `${processing.progress}%` }"></div>
            </div>
        </div>
    </div>
...
import { collection, getDocs, doc, deleteDoc, updateDoc, increment, getDoc } from 'firebase/firestore'; // Note: Added increment and getDoc
...
const fetchTodayData = async () => {
    ...
    try {
        ...
        salesSnap.docs.forEach(docSnap => {
            ...
            if (data.createdBy === currentUser.uid && data.createdAt && data.createdAt.startsWith(todayStr)) {
        ...
        emit('triggerAlert', 'error', 'បរាជ័យ', 'មិនអាចទាញយកទិន្នន័យលក់ថ្ងៃនេះបានទេ');
    } finally {
...
// 🔥 Calculate Summaries Only For VALID (Not Canceled) Sales 🔥
const validSales = computed(() => todaySales.value.filter(s => s.paymentStatus !== 'CANCELED'));

const todayTotalUSD = computed(() => validSales.value.reduce((sum, s) => sum + (s.currency === 'USD' ? Number(s.totalAmount) : 0), 0));
const todayTotalKHR = computed(() => validSales.value.reduce((sum, s) => sum + (s.currency !== 'USD' ? Number(s.totalAmount) : 0), 0));
...
const translateUnit = (u) => {
    ...
    if (unit === 'bottle' || unit === 'bottles') return 'ដប';
    if (unit === 'pack' || unit === 'packs') return 'កញ្ចប់';
    if (unit === 'case' || unit === 'cases') return 'កេះ';
    return u;
};
...
// --- EDIT ---
const editModal = reactive({ isOpen: false, saleId: null, receiptId: '' });
    ...
    try {
        ...
        for (const item of cancelModal.sale.items) {
            ...
            if (stockSnap.exists()) {
                ...
                // We stored qty in retail unit, we need to divide by itemsPerCase if DB uses Case
