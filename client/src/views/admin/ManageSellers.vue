    ...
    <div class="sticky top-0 z-40 bg-[#F4F7FE]/80 backdrop-blur-2xl border-b border-slate-200/60 transition-all px-4 md:px-8 py-5">
      <div class="max-w-[90rem] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-5">
        ...
        <div class="flex items-center gap-4">
           <div class="w-14 h-14 rounded-[1.25rem] bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-500/20 text-white shrink-0 relative overflow-hidden group">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div class="relative w-full sm:w-80 group">
             ...
             <input 
               v-model="searchQuery" 
               type="text" 
               class="w-full pl-12 pr-4 py-3.5 bg-white border border-slate-200/80 rounded-[1.25rem] text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 shadow-sm transition-all placeholder:text-slate-400"
        ...
        >
           ...
           >
              <div class="h-28 relative p-3">
                  <div class="absolute inset-0 bg-gradient-to-br from-indigo-400 via-indigo-500 to-purple-600 rounded-t-[1.75rem] rounded-b-2xl">
              ...
              <div class="px-6 py-4 text-center flex-1 flex flex-col items-center">
                 <h3 class="font-black text-xl text-slate-800 leading-tight mb-1 group-hover:text-indigo-600 transition-colors">{{ seller.fullName }}</h3>
          ...
          <div class="flex min-h-full items-center justify-center p-4">
            <TransitionChild as="template" enter="duration-400 ease-out" enter-from="opacity-0 scale-95 translate-y-10" enter-to="opacity-100 scale-100 translate-y-0" leave="duration-200 ease-in" leave-from="opacity-100 scale-100 translate-y-0" leave-to="opacity-0 scale-95 translate-y-10">
              <DialogPanel class="w-full max-w-4xl transform overflow-hidden rounded-[2rem] bg-white shadow-2xl transition-all relative">
                ...
                <div class="px-8 py-6 flex justify-between items-center relative overflow-hidden border-b border-slate-100 bg-slate-50">
                  <div class="relative z-10 flex items-center gap-4">
                    <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-md bg-gradient-to-br from-indigo-500 to-purple-600 shadow-indigo-500/30">
                ...
                <div class="p-8 bg-white">
                  <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-8 space-y-5">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">ឈ្មោះពេញ (Full Name) <span class="text-rose-500">*</span></label>
                                <input 
                                    ref="nameInput" 
                                    v-model="form.fullName" 
                                    @input="errors.fullName = ''"
                                    type="text" 
                                    :class="errors.fullName ? 'border-rose-500 bg-rose-50/50 focus:ring-rose-500/20' : 'border-slate-200 bg-slate-50 focus:border-indigo-500 focus:bg-white focus:ring-indigo-500/20'"
                                    class="w-full rounded-xl px-4 py-3 font-bold text-sm outline-none transition-all placeholder-slate-400 shadow-inner focus:ring-2" 
                                    placeholder="បញ្ចូលឈ្មោះពេញ..."
                                >
                                <p v-if="errors.fullName" class="text-rose-500 text-[10px] font-bold mt-1.5 ml-1 animate-fade-in flex items-center gap-1">
                            ...
                            <div>
                                <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">ឈ្មោះគណនី (Username) <span class="text-rose-500">*</span></label>
                                <div class="relative group">
                                    <span class="absolute inset-y-0 left-0 pl-4 flex items-center font-bold transition-colors" :class="errors.username ? 'text-rose-500' : 'text-slate-400 group-focus-within:text-indigo-500'">@</span>
                                    <input 
                                        v-model="form.username" 
                                        @input="errors.username = ''"
                                        type="text" 
                                        :class="errors.username ? 'border-rose-500 bg-rose-50/50 text-rose-900 focus:ring-rose-500/20' : 'border-slate-200 bg-slate-50 text-slate-800 focus:border-indigo-500 focus:bg-white focus:ring-indigo-500/20'"
                                        class="w-full rounded-xl pl-9 pr-4 py-3 font-bold text-sm outline-none transition-all placeholder-slate-400 shadow-inner focus:ring-2" 
                                        placeholder="username"
                                    >
                                </div>
                                <p v-if="errors.username" class="text-rose-500 text-[10px] font-bold mt-1.5 ml-1 animate-fade-in flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    {{ errors.username }}
                                </p>
                            </div>
                            ...
                            <div>
                                <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">លេខសម្ងាត់ (Password) <span class="text-rose-500">*</span></label>
                                <input 
                                    v-model="form.password" 
                                    @input="enforceNumberOnly('password')"
                                    type="text" 
                                    maxlength="15"
                                    :class="errors.password ? 'border-rose-500 bg-rose-50/50 focus:ring-rose-500/20' : 'border-slate-200 bg-slate-50 focus:border-indigo-500 focus:bg-white focus:ring-indigo-500/20'"
                                    class="w-full rounded-xl px-4 py-3 font-bold font-mono tracking-widest text-sm outline-none transition-all placeholder-slate-400 shadow-inner focus:ring-2" 
                                    placeholder="យ៉ាងតិច ៦ ខ្ទង់"
                                >
                                <p v-if="errors.password" class="text-rose-500 text-[10px] font-bold mt-1.5 ml-1 animate-fade-in flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    {{ errors.password }}
                                </p>
                            </div>
                            ...
                            <div>
                                <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">តេឡេក្រាម (Telegram) <span class="text-rose-500">*</span></label>
                                <div class="relative group">
                                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center transition-colors" :class="errors.telegram ? 'text-rose-500' : 'text-slate-400 group-focus-within:text-sky-500'">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.48-.94-2.4-1.54-1.06-.7-.37-1.09.23-1.72.16-.16 2.87-2.63 2.92-2.85.01-.03.01-.14-.06-.2-.06-.06-.17-.04-.25-.02-.11.02-1.91 1.2-5.39 3.55-.5.34-.95.51-1.35.5-.44-.01-1.29-.25-1.92-.42-.77-.21-1.37-.32-1.31-.68.03-.18.28-.37.76-.56 3.03-1.32 5.06-2.19 6.09-2.62 2.93-1.21 3.53-1.43 3.93-1.43.09 0 .28.01.4.04.1.03.24.1.33.25.08.16.07.32.07.33z"/></svg>
                                    </span>
                                    <input 
                                        ...
                                        type="text" 
                                        :class="errors.telegram ? 'border-rose-500 bg-rose-50/50 text-rose-900 focus:ring-rose-500/20' : 'border-slate-200 bg-slate-50 text-slate-800 focus:border-sky-500 focus:bg-white focus:ring-sky-500/20'"
                                        class="w-full rounded-xl pl-10 pr-4 py-3 font-bold text-sm outline-none transition-all placeholder-slate-400 shadow-inner focus:ring-2" 
                                        placeholder="username មិនមាន @"
                                    >
                                </div>
                                <p v-if="errors.telegram" class="text-rose-500 text-[10px] font-bold mt-1.5 ml-1 animate-fade-in flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    {{ errors.telegram }}
                                </p>
                            </div>
                            ...
                            <div>
                                <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">លេខទូរស័ព្ទ (Phone)</label>
                                <div class="relative group">
                                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center transition-colors" :class="errors.phoneNumber ? 'text-rose-500' : 'text-slate-400 group-focus-within:text-indigo-500'">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                                    </span>
                                    <input 
                                        v-model="form.phoneNumber" 
                                        @input="enforceNumberOnly('phoneNumber')"
                                        type="text" 
                                        maxlength="12"
                                        :class="errors.phoneNumber ? 'border-rose-500 bg-rose-50/50 text-rose-900 focus:ring-rose-500/20' : 'border-slate-200 bg-slate-50 text-slate-800 focus:border-indigo-500 focus:bg-white focus:ring-indigo-500/20'"
                                        class="w-full rounded-xl pl-10 pr-4 py-3 font-bold text-sm outline-none transition-all placeholder-slate-400 shadow-inner focus:ring-2" 
                                        placeholder="012345678"
                                    >
                                </div>
                                <p v-if="errors.phoneNumber" class="text-rose-500 text-[10px] font-bold mt-1.5 ml-1 animate-fade-in flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    {{ errors.phoneNumber }}
                                </p>
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">អត្តសញ្ញាណប័ណ្ណ (ID Number) <span class="text-rose-500">*</span></label>
                                <div class="relative group">
                                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center transition-colors" :class="errors.idNumber ? 'text-rose-500' : 'text-slate-400 group-focus-within:text-indigo-500'">
                                    ...
                                    <input 
                                        ...
                                        @input="enforceNumberOnly('idNumber')"
                                        type="text" 
                                        maxlength="15"
                                        :class="errors.idNumber ? 'border-rose-500 bg-rose-50/50 text-rose-900 focus:ring-rose-500/20' : 'border-slate-200 bg-slate-50 text-slate-800 focus:border-indigo-500 focus:bg-white focus:ring-indigo-500/20'"
                                        class="w-full rounded-xl pl-10 pr-4 py-3 font-bold font-mono tracking-widest text-sm outline-none transition-all placeholder-slate-400 shadow-inner focus:ring-2" 
                                        placeholder="លេខប៉ុណ្ណោះ (ឧ. 010101010)"
                                    >
                                </div>
                                <p v-if="errors.idNumber" class="text-rose-500 text-[10px] font-bold mt-1.5 ml-1 animate-fade-in flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    {{ errors.idNumber }}
                                </p>
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">អាសយដ្ឋាន (Address)</label>
                                <input v-model="form.address" type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 font-bold text-sm focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder-slate-400 shadow-inner" placeholder="ផ្ទះលេខ, ផ្លូវ, សង្កាត់, ខណ្ឌ, រាជធានី/ខេត្ត...">
                            </div>
                    ...
                    <div class="lg:col-span-4 space-y-5">
                        ...
                        <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[200px]">
                            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center justify-between">
                                <span>រូបថតផ្ទាល់ខ្លួន (Profile)</span>
                            ...
                            <div @click="$refs.profileInput.click()" class="flex-1 border-2 border-dashed border-slate-300 hover:border-indigo-400 bg-slate-50 hover:bg-indigo-50/50 rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden relative group">
                                ...
                                <div v-else class="text-center p-4 group-hover:scale-105 transition-transform">
                                    <div class="w-10 h-10 bg-white shadow-sm rounded-full flex items-center justify-center mx-auto mb-2 text-slate-400 group-hover:text-indigo-500 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                                    </div>
                                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">ជ្រើសរើសរូបភាព</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[200px]">
                            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center justify-between">
                                <span>រូបថតអត្តសញ្ញាណប័ណ្ណ (ID)</span>
                            ...
                            <div @click="$refs.idInput.click()" class="flex-1 border-2 border-dashed border-slate-300 hover:border-indigo-400 bg-slate-50 hover:bg-indigo-50/50 rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden relative group">
                                ...
                                <div v-else class="text-center p-4 group-hover:scale-105 transition-transform">
                                    <div class="w-10 h-10 bg-white shadow-sm rounded-full flex items-center justify-center mx-auto mb-2 text-slate-400 group-hover:text-indigo-500 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0c0 .884-.5 2-2 2h4c-1.5 0-2-1.116-2-2z"></path></svg>
                                    </div>
                                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">ជ្រើសរើសរូបភាព</p>
                                </div>
                            </div>
                ...
                <div class="bg-slate-50 px-8 py-5 flex items-center justify-end gap-3 border-t border-slate-200">
                  <button @click="closeModal" class="px-6 py-3 rounded-xl border border-slate-200 text-slate-600 font-bold hover:bg-white transition-all text-sm bg-slate-100 shadow-sm">បោះបង់ (Cancel)</button>
                  <button @click="submitSeller" :disabled="isSubmitting" class="text-white px-8 py-3 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95 text-sm min-w-[160px] bg-slate-800 hover:bg-slate-900 shadow-slate-900/20">
              ...
              <DialogPanel class="w-full max-w-2xl transform overflow-hidden rounded-[2.5rem] bg-slate-50 shadow-2xl transition-all relative border border-white">
                ...
                <div class="h-44 bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 relative overflow-hidden">
                   ...
                   <div class="absolute top-6 left-8">
                      ...
                      <div class="flex items-center gap-2 bg-white/10 w-fit px-3 py-1.5 rounded-lg border border-white/10 backdrop-blur-sm">
                         ...
                         <span class="text-white font-bold text-xs">Active Account</span>
                      </div>
                   </div>
                </div>
                ...
                <div class="px-8 relative flex flex-col sm:flex-row justify-between sm:items-end -mt-16 mb-6 gap-4">
                   ...
                   <div class="flex gap-2.5 relative z-10 w-full sm:w-auto">
                      ...
                      <button @click="navigateToSalesHistory(selectedSeller)" class="flex-1 sm:flex-none px-6 py-3 bg-slate-800 text-white rounded-xl font-bold shadow-lg hover:bg-slate-900 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 text-sm active:scale-95">
                ...
                <div class="px-8 pb-8">
                   ...
                   <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                      <div class="bg-[#F8FAFC] p-4 rounded-2xl border border-slate-200/60 flex items-center gap-4">
                         <div class="w-12 h-12 bg-white text-indigo-600 rounded-xl flex items-center justify-center border border-slate-100 shadow-sm">
                      ...
                      <div v-if="selectedSeller?.password" class="bg-[#F8FAFC] p-4 rounded-2xl border border-slate-200/60 flex items-center gap-4">
                         <div class="w-12 h-12 bg-white text-slate-500 rounded-xl flex items-center justify-center border border-slate-100 shadow-sm">
...
// 🌟 Real-time Number Enforcer 🌟
const enforceNumberOnly = (field) => {
    form[field] = form[field].replace(/[^0-9]/g, '');
    errors[field] = ''; // Clear error on type
};
...
const resetForm = () => {
  Object.assign(form, { fullName: '', username: '', address: '', telegram: '', phoneNumber: '', idNumber: '', password: '', profileFile: null, idCardFile: null });
...
const submitSeller = async () => {
  ...
  if (!form.fullName.trim()) { errors.fullName = 'សូមបញ្ចូលឈ្មោះពេញ'; hasError = true; }
  if (!form.username.trim()) { errors.username = 'សូមបញ្ចូលឈ្មោះគណនី'; hasError = true; }
  if (!form.telegram.trim()) { errors.telegram = 'សូមបញ្ចូលតេឡេក្រាម'; hasError = true; }
  if (!form.idNumber.trim()) { errors.idNumber = 'សូមបញ្ចូលអត្តសញ្ញាណប័ណ្ណ'; hasError = true; }

  // 2. Strict Validations (Password & Phone)
  if (!form.password.trim()) { 
      errors.password = 'សូមបញ្ចូលលេខសម្ងាត់'; 
      hasError = true; 
  } else if (form.password.length < 6) {
      errors.password = 'លេខសម្ងាត់ត្រូវមានយ៉ាងតិច ៦ ខ្ទង់';
      hasError = true;
  }

  if (form.phoneNumber.trim()) {
      if (!form.phoneNumber.startsWith('0')) {
          errors.phoneNumber = 'លេខទូរស័ព្ទត្រូវផ្ដើមដោយលេខ 0';
          hasError = true;
      } else if (form.phoneNumber.length < 9) {
          errors.phoneNumber = 'លេខទូរស័ព្ទត្រូវមានយ៉ាងតិច ៩ ខ្ទង់';
          hasError = true;
      }
  }
  ...
  const cleanIdNumber = form.idNumber.replace(/\s+/g, ''); 
  const cleanPhone = form.phoneNumber ? form.phoneNumber.replace(/\s+/g, '') : ''; 
  const cleanTelegram = form.telegram.replace('@', '').trim(); 
  ...
  try {
     ...
     if (errCode === "ID_EXISTS") {
         errors.idNumber = 'អត្តសញ្ញាណប័ណ្ណនេះមានក្នុងប្រព័ន្ធរួចហើយ!';
     } else if (errCode === "USERNAME_EXISTS") {
         errors.username = 'ឈ្មោះគណនីនេះមានអ្នកប្រើប្រាស់រួចហើយ!';
     } else {
